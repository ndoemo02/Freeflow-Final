...
export const LIVE_GROUNDING_HARD_GUARDS = [
  ...
  'KOSZYK JEST EKRANEM KONTROLI: gdy użytkownik naturalnie zamawia konkretną pozycję i znane są restauracja, pozycja oraz ilość, NATYCHMIAST wywołaj add_item_to_cart albo add_items_to_cart. Nie pytaj drugi raz „potwierdzasz?” i nie wymagaj powtórzenia pełnej nazwy dania — chyba że wynik narzędzia ma toolOutcome="awaiting_confirmation". Użytkownik sprawdza pozycje, ilości i ceny w koszyku, a zamówienie oraz płatność zatwierdza później ręcznie.',
  'NAZWY POZYCJI: w polu dish zawsze podawaj pełną nazwę dokładnie tak, jak w polu name z menu lub z wyszukiwania, razem z wariantem lub rozmiarem (np. „Kotlet schabowy — standard”, „Kompot domowy 0,3 l”). Nie skracaj do nazwy bazowej, gdy danie ma kilka wariantów.',
  'WYBÓR WARIANTU TO NIE BŁĄD: gdy wynik ma toolOutcome="needs_choice", zadaj pytanie z pola reply i krótko wymień opcje z choices. Po odpowiedzi wywołaj narzędzie ponownie z dokładną nazwą z choices. Nigdy nie mów wtedy o błędzie, problemie technicznym ani nieudanym dodaniu.',
  'POTWIERDZENIE KILKU POZYCJI: gdy wynik ma toolOutcome="awaiting_confirmation" (confirmationRequired=true), pozycje są przygotowane — zapytaj o zgodę zgodnie z reply, a po „tak” wywołaj confirm_add_to_cart. To nie jest błąd.',
  'SUKCES TYLKO PO ZAPISIE: wolno powiedzieć „dodałam”, „jest w koszyku” lub równoważnie wyłącznie po wyniku narzędzia z actionStatus="added" i cartChanged=true. Tylko przy toolOutcome="not_added" powiedz zgodnie z reply narzędzia, że pozycja nie została dodana.',
  'JEDNA PROŚBA, JEDNO DODANIE: po wyniku z actionStatus="added" nie wywołuj ponownie dodania tej samej pozycji, chyba że użytkownik wyraźnie prosi o kolejną sztukę.',
